// The Claude brain. Ported VERBATIM from the proven prototype (prototype/weekly-read.mjs).
// This prompt is the product. Do not "improve" it without re-running the seed set —
// the buried-vs-stated behaviour and the "generates nothing" constraint are load-bearing.
...
export type Thread = {
  ...
  recurrence: number;
  guises: string[];
  fragment_timestamps: string[];
};

export type WeeklyRead = {
  threads: Thread[];
  stated_subject: string;
  buried_thread: string;
  primary_reflection: string;
  nothing_recurred: boolean;
};

export type Fragment = { ts: string; text: string };

export function buildSystemPrompt(practice: string): string {
  return `
...
a buried thread. Never flatter, never therapise, never tell them what it "means" for them or
what they "should" do. Name what is there, in their words.
...
export function buildUserMessage(practice: string, fragments: Fragment[]): string {
  return [
    `PRACTICE: ${practice}`,
    "",
