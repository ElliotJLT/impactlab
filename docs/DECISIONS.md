...
## 011 — Warm two-theme palette + animated signature gradient bloom
**When:** (this session) · **Who:** Andrei — **touches the shared design system (Decision 004); needs reconciling with Samantha's scaffold (008)**
**Decision:** Reworked `docs/DESIGN.md` to exactly two warm, system-driven themes (light/dark on `prefers-color-scheme`, no in-app toggle, dark as no-preference default) plus one signature gradient — a vibrant warm mesh "bloom" (orange→red in dark, warm peach in light) that fills the top of the canvas and fades out by ~56%, drifting very slowly (~36s translate/scale/hue cycle) on a fixed `body::before`. Accent is "Ember" warm orange (`#F0803F` dark / `#A84A0E` light), drawn from the bloom. Visual reference: a Zentra-style onboarding gradient. All contrast pairs recomputed and pass WCAG AA.
**Why:** The old palette (cold near-black + aqua-teal) read techy and intimidating, against "Sleep On It"'s calm, human intent. An earlier iteration used a time-of-day tint; dropped it — too clever, and it made the look non-deterministic for a 3-minute demo. The bloom is accessible because of one layout rule from the reference: the gradient is empty atmosphere, all body/muted text lives on the solid base or on solid `--surface` cards (both fully verified); the display line sits low in the faded bloom (8.03:1 dark / 8.73:1 light); nothing sits on the bright hotspot (display there is only 2.75:1). The slow drift is a logged exception to "don't animate what the user didn't touch" — it's identity, not response, and too slow to distract; the animation is confined to the top so the lower text zone stays base throughout, and `prefers-reduced-motion` freezes it. Reverts cleanly — palette, `--bloom`, and the `body::before` block live entirely in the token section. Amends Decision 004's "one accent colour"; still one accent, now warm, plus the one sanctioned gradient. Note: landed after Samantha's scaffold (008), whose `globals.css`/Tailwind v4 tokens code against the previous `DESIGN.md` palette — the app CSS needs updating to these tokens for the two to match (follow-up).
